...
import {
  ...
  fitScore, valueScore, nodeEffTps, rateStats, assetMonthlyDepreciation,
...
function Capacity({ S, f }) {
  ...
  const localMonthlyFull = c.nodes * (nodeMonthlyRun(S) + (S.owns === 'yes' ? 0 : assetMonthlyDepreciation(nodeCapex(S), S.dep)))
  ...
  const effBandTxt = EFF_BAND[0] + '–' + EFF_BAND[1] + '×'
  let chipCls, chipTxt, band
  if (mult >= EFF_BAND[0] && mult <= EFF_BAND[1]) { chipCls = 'g'; chipTxt = 'in band'; band = 'right inside the cited ' + effBandTxt + ' on-prem efficiency range.' }
  else if (mult > EFF_BAND[1]) { chipCls = 'g'; chipTxt = 'above band'; band = 'beats Dell/ESG’s ' + effBandTxt + ' — the node is heavily utilized.' }
  else { chipCls = 'b'; chipTxt = 'below band'; band = 'under Dell/ESG’s ' + effBandTxt + ' — raise utilization, local share, or use a smaller node to reach it.' }
